# DevSmith Flask Sample App

Minimal Flask application demonstrating DevSmith cross-repo logging integration.

## Features

- ✅ DevSmith logger with batch ingestion
- ✅ Flask extension for automatic request/response logging
- ✅ Route decorator for custom logging
...
DEVSMITH_SERVICE_NAME=flask-sample
```
...
```
**Expected Logs**: None (skipped by extension)
...
3. Filter by service: `flask-sample`
...
The `@log_route` decorator adds automatic logging to specific routes:

```python
@app.route('/api/custom')
@devsmith_ext.log_route(context={'feature': 'custom'}, tags=['custom', 'api'])
def custom_endpoint():
    # Automatically logs: DEBUG - Executing custom_endpoint
    return jsonify({'status': 'ok'})
```

## Log Levels Used

- **DEBUG**: Route execution, detailed operations
...
Reduce buffer size in `app.py`:
```python
logger = DevSmithLogger(
    # ...
    buffer_size=50,  # Was 100
    flush_interval=3.0  # Was 5.0 (flush more frequently)
)
```
...
- Integrate with your existing error tracking (Sentry, etc.)
...
- [Python Logger Documentation](../../../python/README.md)

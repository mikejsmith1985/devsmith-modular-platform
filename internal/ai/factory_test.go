...
func (m *MockEncryptionService) DecryptAPIKey(encrypted string, userID int) (string, error) {
	...
	if m.decryptError != "" {
		return "", &EncryptionError{Message: m.decryptError}
	}

	decrypted, exists := m.decryptedKeys[encrypted]
	if !exists {
		return "", &EncryptionError{Message: "key not found"}
	}

	return decrypted, nil
}

// LLMConfig represents a user's LLM configuration (to be defined in repository)

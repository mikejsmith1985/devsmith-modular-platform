...
type TrendServiceInterface interface {
	...
	AnalyzeTrend(ctx context.Context, metricType models.MetricType, service string, start, end time.Time) (*models.TrendAnalysis, error)
}
...
type AnomalyServiceInterface interface {
	...
	DetectAnomalies(ctx context.Context, metricType models.MetricType, service string, start, end time.Time) ([]models.Anomaly, error)
}
...
type TopIssuesServiceInterface interface {
	...
	GetTopIssues(ctx context.Context, service, level string, start, end time.Time, limit int) ([]models.IssueItem, error)
}
...
type ExportServiceInterface interface {
	...
	ExportData(ctx context.Context, metricType models.MetricType, service string, start, end time.Time, format string) ([]byte, error)
}
